        ...
        <article key={message.id} className={`message message-${message.role}`}>
          <header>
            ...
            <div className="message-header-meta">
              ...
              {message.role === 'user' && (message.metadata?.usage?.inputTokens ?? 0) > 0 && (
                <span className="token-badge token-badge-input" title="Total input tokens sent to the LLM">
                  ↑ {(message.metadata!.usage!.inputTokens!).toLocaleString()} tokens
                </span>
              )}
              {message.role === 'assistant' && (message.metadata?.usage?.outputTokens ?? 0) > 0 && (
                <span className="token-badge token-badge-output" title="Total output tokens generated by the LLM">
                  ↓ {(message.metadata!.usage!.outputTokens!).toLocaleString()} tokens
                </span>
              )}
            </div>
          </header>
